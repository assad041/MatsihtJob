...
class Super_admin {
    ...
    public function delete_org($id) {
        $sql="UPDATE organizations SET status='0' WHERE org_id='$id' ";
        if(mysqli_query($this->db_connect, $sql)) {
            $message="Congratulation ! Client delete successfully";
            return $message;
        } else {
            die('Query problem'.  mysqli_error($this->db_connect) );
        }
  ...
  public function delete_academics($id) {
        $sql="UPDATE academics SET status='0' WHERE aca_id='$id' ";
        if(mysqli_query($this->db_connect, $sql)) {
            $message="Congratulation ! Items delete successfully";
            return $message;
        } else {
            die('Query problem'.  mysqli_error($this->db_connect) );
        }
    ...
    public function delete_applicant($id) {
        $sql="UPDATE applicants SET status='0' WHERE app_id='$id' ";
        if(mysqli_query($this->db_connect, $sql)) {
            $message="Congratulation ! Applicant delete successfully";
            return $message;
        } else {
            die('Query problem'.  mysqli_error($this->db_connect) );
        }
    ...
    public function delete_category($id) {
        $sql="UPDATE categorys SET status='0' WHERE cat_id='$id' ";
        if(mysqli_query($this->db_connect, $sql)) {
            $message="Congratulation ! Category Items delete successfully";
            return $message;
        } else {
            die('Query problem'.  mysqli_error($this->db_connect) );
        }
    ...
    public function select_inbox($id,$apply_id) {
        $sql="SELECT * FROM inbox WHERE inbox_id='$id' ";
        if(mysqli_query($this->db_connect, $sql)) {
            $query_result=mysqli_query($this->db_connect, $sql);
            $sql="UPDATE job_apply SET  new='0' WHERE apply_id='$apply_id'";
        ...
        if( $_SESSION['captcha']['code']==$cap_img ){
            if($password==$rpassword){
                ...
                if(!mysqli_fetch_assoc($query_result)){
                     ...
                     $sql="INSERT INTO applicants (first_name,last_name,gender,news_subscribe,date_of_birth,cell_phone,email,password,paddress,skill_test,skype_id) VALUES ('$first_name','$last_name','$gender','$subscribe','$date_of_birth','$cell_phone', '$email','$password','$v_code','','')";
    ...
    public function validate_applicant($v_code){
        ...
        if($app=mysqli_fetch_assoc($query_result)) {
            $sql="UPDATE applicants SET paddress='', status='2' WHERE paddress='$v_code' ";
            if(mysqli_query($this->db_connect, $sql)) {
                $message="done";
                session_start();
                $_SESSION['applicant_name']=$app['email'];
                $_SESSION['applicant_id']=$app['app_id'];
                return $message;
            } else {
                die('Query problem'.mysqli_error($this->db_connect) );
            }
    ...
    public function update_applicant_personal_data($data) {
        ...
        $sql="UPDATE applicants SET first_name='$first_name',last_name='$last_name',fname='$fname',mname='$mname',maddress='$maddress',paddress='$paddress',gender='$gender',date_of_birth='$date_of_birth',cell_phone='$cell_phone',email='$email',job_category='$job_category',skill_test='',skype_id='' WHERE app_id='$id' ";
        if(mysqli_query($this->db_connect, $sql)) {
    ...
    public function save_academic($data) {
        ...
        if($status==5){
            ...
            if(mysqli_fetch_assoc($query_result)) {
                 ...
                 $sql="INSERT INTO academics (app_id,degree_name,institute_university,passing_year,result,comments,entry_by,status) VALUES ('$app_id','$degree_name','$institute_university','$passing_year','2.1reg','1','$entry_by','$status' )";
            }
   ...
   public function uploadpicture($data){
    ...
    public function validate_organization($v_code){
        ...
        $query_result=mysqli_query($this->db_connect, $sql);
        if($org=mysqli_fetch_assoc($query_result)) {
            $sql="UPDATE organizations SET contact_email='', status='2' WHERE contact_email='$v_code' ";
            if(mysqli_query($this->db_connect, $sql)) {
                session_start();
                $_SESSION['client_name']=$org['org_name'];
                $_SESSION['client_id']=$org['org_id'];
                $message="done";
                return $message;
            } else {
                die('Query problem'.mysqli_error($this->db_connect) );
    ...
    public function send_organaization_response($data){
        ...
        $result=$this-> select_apply_job_accepted_list($id,$job_info["org_id"]);
        
        while ($row=mysqli_fetch_assoc($result)) {
            $applicants=$this->select_applicant_by_id($row["app_id"]);
            $email_to = $applicants["email"];
            $email_from="info@matsihtjob.com";
            $email_subject = $subject;
            $email_message = $message;
            @mail($email_to, $email_subject, $email_message, $headers);
            $sql="INSERT INTO inbox (inbox_to,inbox_from,subject,message,status) VALUES ('$email_to','$email_from','$email_subject','$email_message','1' )";
            mysqli_query($this->db_connect,$sql);
            $inbox_id=$this->db_connect->insert_id;
            $app_id=$row["app_id"];
            $sql="UPDATE job_apply SET inbox='$inbox_id', new='1' WHERE post_id='$id' AND app_id='$app_id' AND status='1' ";
            mysqli_query($this->db_connect, $sql);
        }
    ...
    public function select_apply_job_canceled_list($post_id,$org_id){
        $sql="SELECT * FROM job_apply WHERE post_id='$post_id' AND org_id='$org_id' AND status='0' ORDER BY `job_apply`.`star` DESC";
        if(mysqli_query($this->db_connect, $sql)) {
            $query_result=mysqli_query($this->db_connect, $sql);
    ...
    public function get_number_of_canceled_candidate($post_id){
        $sql="SELECT * FROM job_apply WHERE post_id='$post_id' AND status='0' ";
        if(mysqli_query($this->db_connect, $sql)) {
           $query_result=mysqli_query($this->db_connect, $sql);
            return mysqli_num_rows($query_result);
        } else {
