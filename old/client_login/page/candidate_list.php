<?php
    ...
    if(isset($_GET['action'])){
        $action=$_GET['action'];
        $a=$_GET['can'];
        $query_result=$ob_sup_admin->selection_apply_job($action,$a,$id);

    }

    $query_result=$ob_sup_admin->select_apply_job_candidate_list($id,$_SESSION['client_id']);
...
?>
        ...
        <div class="recent-job"><!-- Start Job -->
            <div class="container">
                <h4><i class="fa fa-briefcase"></i> Candidate List</h4>
                        ...
                        while ( $row = mysqli_fetch_assoc($query_result) ) {
                            ...
                            <div class="recent-job-list"><!-- Tabs content -->
                                <div class="col-md-1 job-list-logo">
                                    <img src="../assets/candidate/'.$row["app_id"].'.jpg" class="img-responsive" alt="dummy-joblist" />
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="col-md-6 job-list-type">
                                             <a class="btn-view-job btn-green" href="?id='.$id.'&&action=ac&&can='.$row["app_id"].'" >&nbsp;Accept&nbsp;</a>
                                        
                                            
                                        </div>
                                        <div class="col-md-6 job-list-type">
                                            <a class=" btn-apply-job" href="?id='.$id.'&&action=ce&&can='.$row["app_id"].'" >Cencel</a>
                                        </div>
                                    </div>
                                </div>
